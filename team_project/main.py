...
# csv파일 생성
file = open('apartment.csv', 'w', encoding="utf-8-sig", newline='')
writer = csv.writer(file)
writer.writerow(['거래', '면적', '가격', '동', '상세 설명'])
...
browser.find_element(By.ID, 'query').send_keys("한남동 나인원한남", Keys.ENTER)
...
for i, data in enumerate(datas):
    ...
    writer.writerow([type, spec[0].string, price, where, spec[1].string])
